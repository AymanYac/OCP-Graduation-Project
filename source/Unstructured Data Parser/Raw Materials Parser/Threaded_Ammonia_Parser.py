...
class processeur(Thread):

    def __init__(self,wb,pdf_name):
        Thread.__init__(self)
        self.wb = wb
        self.pdf_name = pdf_name

    def run(self):
    	matrix = open(self.pdf_name, "rb")
    	day=self.pdf_name.split("/")[1].split(".")[0].split("_")[-1].split("-")[2]
    	month=self.pdf_name.split("/")[1].split(".")[0].split("_")[-1].split("-")[1]
    	if day+month!="1808":
    		...
    		day = post.day
    		title=str(day)+" "+month
    		fill(self.wb,matrix,title,14,post.month,post.day)
    	else:
			post = datetime.datetime(2016,int(month),int(day)) - datetime.timedelta(days = 14)
			month = ["Janvier","Fevrier","Mars","Avril","Mai","Juin","Juillet","Aout","Septembre","Octobre","Novembre","Decembre"][post.month-1]
			day = post.day
			...
			day = post.day
			title=str(day)+" "+month
			fill(self.wb,matrix,title,7,post.month,post.day)
			day=self.pdf_name.split("/")[1].split(".")[0].split("_")[-1].split("-")[2]
			month=self.pdf_name.split("/")[1].split(".")[0].split("_")[-1].split("-")[1]
			...
			day = post.day
			title=str(day)+" "+month
			fill(self.wb,matrix,title,0,post.month,post.day)
...
def main(argv):
	...
	for pdfname in sorted(glob.glob(sys.argv[1]+"/*.pdf")):
		processeur(wb,pdfname).start()
	wb.save("out.xlsx")
