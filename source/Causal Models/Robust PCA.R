ArabHecPP = read.csv('~/Desktop/OCP/ExtData/arableLandHectarparPerson.csv')[,c("Country.Name","X2013")]
names(ArabHecPP) = c("Country.Name","ArabHecPP")
CropProd = read.csv('~/Desktop/OCP/ExtData/crop_production.csv')[,c("Country.Name","X2013")]
names(CropProd) = c("Country.Name","CropProd")
PercArabLand = read.csv('~/Desktop/OCP/ExtData/%arableLand.csv')[,c("Country.Name","X2013")]
names(PercArabLand) = c("Country.Name","PercArabLand")
FertConsHecArab = read.csv('~/Desktop/OCP/ExtData/FertConsupHecArabl.csv')[,c("Country.Name","X2013")]
names(FertConsHecArab) = c("Country.Name","FertConsHecArab")
GDPC = read.csv('~/Desktop/OCP/ExtData/GDPperCapita.csv')[,c("Country.Name","X2013")]
names(GDPC) = c("Country.Name","GDPC")
Pop = read.csv('~/Desktop/OCP/ExtData/population.csv')[,c("Country.Name","X2013")]
names(Pop) = c("Country.Name","Pop")
RPop = read.csv('~/Desktop/OCP/ExtData/ruralPopulation.csv')[,c("Country.Name","X2013")]
names(RPop) = c("Country.Name","RPop")
L1 = ls()
#Reduce(intersect,(lapply(lapply(list,get),names)))
L2 = unique(CropProd$Country.Name)
ArabHecPP=ArabHecPP[which(ArabHecPP$Country.Name %in% L2),]
FertConsHecArab=FertConsHecArab[which(FertConsHecArab$Country.Name %in% L2),]
GDPC=GDPC[which(GDPC$Country.Name %in% L2),]
PercArabLand=PercArabLand[which(PercArabLand$Country.Name %in% L2),]
Pop=Pop[which(Pop$Country.Name %in% L2),]
RPop = RPop[which(RPop$Country.Name %in% L2),]
DF = Reduce(merge,lapply(L1,get))
DF = DF[rowSums(is.na(DF)) == 0,]
rm(list = L1)
rm(L1,L2)
rownames(DF) <- DF[,1]
DF = DF[,c("Country.Name","ArabHecPP","CropProd","GDPC","PercArabLand","Pop","RPop","FertConsHecArab")]
